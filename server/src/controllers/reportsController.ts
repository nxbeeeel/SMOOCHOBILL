...
export const getDailySalesSummary = async (req: Request, res: Response) => {
  try {
    ...
    const salesData = await databaseManager.all(`
      SELECT 
        o.id,
        o.order_number,
        o.customer_name,
    ...
    const orderTypeBreakdown = await databaseManager.all(`
      SELECT 
        order_type,
        COUNT(*) as order_count,
        SUM(total_amount) as total_amount
    ...
    const topProducts = await databaseManager.all(`
      SELECT 
        oi.product_name,
        oi.product_id,
    ...
    throw createError('Failed to get daily sales summary', 500);
  }
};
...
export const getMonthlyReport = async (req: Request, res: Response) => {
  try {
    ...
    const monthlyData = await databaseManager.all(`
      SELECT 
    ...
    // Monthly summary
    const monthlySummary = await databaseManager.get(`
      SELECT 
        COUNT(DISTINCT o.id) as total_orders,
        COUNT(CASE WHEN o.payment_status = 'completed' THEN 1 END) as completed_orders,
        SUM(CASE WHEN o.payment_status = 'completed' THEN o.total_amount ELSE 0 END) as total_revenue,
    ...
    const categoryPerformance = await databaseManager.all(`
      SELECT 
    ...
    throw createError('Failed to get monthly report', 500);
  }
};
...
export const getProfitAnalysis = async (req: Request, res: Response) => {
  try {
    ...
    // Get sales data with cost analysis
    const profitData = await databaseManager.all(`
      SELECT 
        o.id,
        o.order_number,
    ...
    const dailyProfit = await databaseManager.all(`
      SELECT 
        DATE(o.created_at) as date,
    ...
    const productProfit = await databaseManager.all(`
      SELECT 
        oi.product_name,
        oi.product_id,
        SUM(oi.quantity) as total_quantity,
    ...
    throw createError('Failed to get profit analysis', 500);
  }
};

export const getStockUsageReport = async (req: Request, res: Response) => {
  try {
    ...
    // Stock usage summary
    const stockUsage = await databaseManager.all(`
      SELECT 
        i.id,
        i.name,
        i.category_id,
    ...
    const dailyStockMovement = await databaseManager.all(`
      SELECT 
    ...
    const categoryStockUsage = await databaseManager.all(`
      SELECT 
        c.id,
        c.name as category_name,
        c.color as category_color,
    ...
    throw createError('Failed to get stock usage report', 500);
  }
};

export const exportDailySalesPDF = async (req: Request, res: Response) => {
  try {
    const { date } = req.query;
    const targetDate = date ? new Date(date as string) : new Date();
    const dateStr = targetDate.toISOString().split('T')[0];
    ...
    throw createError('Failed to export daily sales PDF', 500);
  }
};
